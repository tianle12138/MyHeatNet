...
def get_palette():
    ...
    bicycle = [190, 153, 153]
    unlabelled = [0, 0, 0]
    palette = np.array([road, sidewalk, building, curb, fench, pole, vegetation, terrain, sky, person, car, bicycle, unlabelled])
    return palette
...
def visualize(image_name, predictions, weight_name):
    ...
    for (i, pred) in enumerate(predictions):
        ...
        ####################################################
        img = np.asarray(Image.fromarray(img).resize((352, 288)), dtype=np.float32)
        ####################################################
    ...
    #     pred = predictions[i].cpu().numpy()
    #     pred = pred * 10
    #     cv2.imwrite('demo_results/Pred_' + weight_name + '_' + image_name,pred)
    #     # img.save('demo_results/Pred_' + weight_name + '_' + image_name)
    #     # img = np.zeros((pred.shape[0], pred.shape[1], 3), dtype=np.uint8)
    #     # for cid in range(0, len(palette)): # fix the mistake from the MFNet code on Dec.27, 2019
    #     #     img[pred == cid] = palette[cid]
...
def compute_results(conf_total):
    ...
    for cid in range(start_index, n_class): # cid: class id
        ...
        if (conf_total[start_index:, start_index:].sum()) == 0:
            accuracy_per_class[cid] = np.nan
        else:
            TP = conf_total[cid, cid]
            FN = conf_total[cid, start_index:].sum() - conf_total[cid, cid]
